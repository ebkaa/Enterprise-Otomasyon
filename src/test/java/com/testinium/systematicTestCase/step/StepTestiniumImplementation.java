...
public class StepTestiniumImplementation {
    ...
    public void loginWithTable(Table table) {
        ...
        for(int i = 0; i < listEmail.size(); i++){
            ...
            methods.doesUrl("https://dev.testinium.com",80,"equal");
        }
    }
    ...
    public void controlElemenListText(String key, String text){
        ...
        for (WebElement element: elements) {
            ...
            Assert.assertTrue("Listede deneme kelimesi yok",elementText.toLowerCase(Locale.ROOT).contains(text));
    ...
    public void verifyPlanRunning() {
        methods.checkElementVisible(methods.getBy("lastExecutionInNewSuite"), 30);
        String urlPlan = methods.getCurrentUrl().trim();
        methods.checkElementVisible(methods.getBy("lastExecutionResultInNewSuite"), 30);
        methods.waitBySeconds(1);
        //methodsPage.checkElementExistWithUrl(methodsPage.getBy("testRunningInNewSuite"),10,20, urlPlan,"Test başlatılamadı.");
        methods.checkElementExistWithUrl(methods.getBy("testRunningResultInNewSuite"), 20, 15, urlPlan, "Test beklenen sürede tamamlanamadı.");
    }
